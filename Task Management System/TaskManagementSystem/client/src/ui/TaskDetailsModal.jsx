          ...
          {ENABLE_AI_ASSIST ? (
            <section className="taskModalSection">
              <div className="taskModalSectionHeader">
                <h3>AI Assist</h3>
                <span className="pill">
                  {aiStatus?.mode ? String(aiStatus.mode) : "Template"}
                </span>
              </div>
              ...
              <div className="taskModalSectionBody">
                <div className="muted small">
                  {aiStatus?.message
                    ? String(aiStatus.message)
                    : "Template mode (no external AI calls by default)."}
                </div>
                ...
                <div className="inlineRow">
                  <input
                    className="input"
                    placeholder="Optional prompt (e.g., summarize blockers, suggest next step)…"
                    value={aiPrompt}
                    ...
                    onClick={async () => {
                      ...
                      try {
                        const res = await api.post(
                          "/api/ai/task-assist",
                          { taskId: task.id, prompt: aiPrompt || null },
                          { __skipOffline: true }
                        );
                ...
                {aiResult?.suggestedActions?.length ? (
                  <>
                    <div className="muted small">Suggested next actions</div>
                    <div className="labelRow">
                      {aiResult.suggestedActions.map((a) => (
                        <span key={a} className="labelChip">
                          {a}
                        </span>
                      ))}
